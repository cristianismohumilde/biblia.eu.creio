...
const getWitnessFromData = (data) => {
  const literalEntry = (data.literalTranslations || []).find((entry) => entry.lang === "geez");
  const transliterationFromTokens = (data.tokens || [])
    .filter((token) => token.lang === "geez" && token.transliteration && token.transliteration !== "-")
    .map((token) => token.transliteration.trim())
    .filter(Boolean)
    .join(" ");

  const witnessList = Array.isArray(data.geezWitnesses) ? data.geezWitnesses : [];
  let witness = witnessList.find((item) => item.id === WITNESS_ID || item.id === "ethiopic" || item.id === "base") || witnessList[0];

  if (witness) {
    return {
      label: witness.label && witness.label !== "Texto-base" ? witness.label : "Tradição Etíope Clássica",
      text: witness.text || data.sourceTexts?.geez || "Sem texto disponível.",
      transliteration:
  ...
  return {
    label: "Tradição Etíope Clássica",
    text: data.sourceTexts?.geez || "Sem texto disponível.",
...
const getGeezTokens = (data) => {
  const tokens = Array.isArray(data.tokens) ? data.tokens : [];
  return tokens.filter((token) => token.lang === "geez");
};
...
const renderTokenRows = (tokens) => {
  ...
  if (!tokens.length) {
    ...
    tr.innerHTML = '<td colspan="8">Nenhum token ge\'ez disponível para este verso.</td>';
...
const renderVerse = (data) => {
  ...
  const tokens = getGeezTokens(data);
