...
const getWitnessFromData = (data) => {
  const literalEntry = (data.literalTranslations || []).find((entry) => entry.lang === "aramaic");
  const transliterationFromTokens = (data.tokens || [])
    .filter((token) => token.lang === "aramaic" && token.transliteration && token.transliteration !== "-")
    .map((token) => token.transliteration.trim())
    .filter(Boolean)
    .join(" ");

  const witnessList = Array.isArray(data.aramaicWitnesses) ? data.aramaicWitnesses : [];
  let witness = witnessList.find((item) => item.id === WITNESS_ID || item.id === "base") || witnessList[0];

  if (witness) {
    return {
      label: witness.label && witness.label !== "Texto-base" ? witness.label : "Targum de Onkelos",
      text: witness.text || data.sourceTexts?.aramaic || "Sem texto disponível.",
      transliteration:
  ...
  return {
    label: "Targum de Onkelos",
    text: data.sourceTexts?.aramaic || "Sem texto disponível.",
...
const getAramaicTokens = (data) => {
  const tokens = Array.isArray(data.tokens) ? data.tokens : [];
  return tokens.filter((token) => token.lang === "aramaic");
};
...
const renderTokenRows = (tokens) => {
  ...
  if (!tokens.length) {
    ...
    tr.innerHTML = '<td colspan="8">Nenhum token aramaico disponível para este verso.</td>';
...
const renderVerse = (data) => {
  ...
  const tokens = getAramaicTokens(data);
