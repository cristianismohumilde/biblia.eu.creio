...
const els = {
  ...
  statLexicon: document.querySelector("#stat-lexicon"),
...
const getWitnessFromData = (data) => {
  const literalEntry = (data.literalTranslations || []).find((entry) => entry.lang === "armenian");
  const transliterationFromTokens = (data.tokens || [])
    .filter((token) => token.lang === "armenian" && token.transliteration && token.transliteration !== "-")
    .map((token) => token.transliteration.trim())
    .filter(Boolean)
    .join(" ");

  const witnessList = Array.isArray(data.armenianWitnesses) ? data.armenianWitnesses : [];
  let witness = witnessList.find((item) => item.id === WITNESS_ID) || witnessList[0];

  if (witness) {
    return {
      label: witness.label && witness.label !== "Texto-base" ? witness.label : "Versão Armênia Antiga",
      text: witness.text || data.sourceTexts?.armenian || "Sem texto disponível.",
      transliteration:
  ...
  return {
    label: "Versão Armênia Antiga",
    text: data.sourceTexts?.armenian || "Sem texto disponível.",
...
const getArmenianTokens = (data) => {
  const tokens = Array.isArray(data.tokens) ? data.tokens : [];
  return tokens.filter((token) => token.lang === "armenian");
};
...
const renderStats = (tokens) => {
  ...
  const withLexicon = tokens.filter((token) => (token.lexicon && token.lexicon !== "-") || (token.strong && token.strong !== "-")).length;
  ...
  if(els.statLexicon) els.statLexicon.textContent = String(withLexicon);
...
const renderTokenRows = (tokens) => {
  ...
  if (!tokens.length) {
    ...
    tr.innerHTML = '<td colspan="8">Nenhum token armênio disponível para este verso.</td>';
...
const renderVerse = (data) => {
  ...
  const tokens = getArmenianTokens(data);

  els.referenceTitle.textContent = `${bookName} ${data.ref.chapter}:${data.ref.verse}`;
  els.witnessMeta.textContent = witness.label;
