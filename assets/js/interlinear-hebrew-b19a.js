...
const els = {
  ...
  statExplanation: document.querySelector("#stat-explanation"),
...
const WITNESS_ID = "leningradensis";
...
const getWitnessFromData = (data) => {
  const literalEntry = (data.literalTranslations || []).find((entry) => entry.lang === "hebrew");
  ...
  const witnessList = Array.isArray(data.hebrewWitnesses) ? data.hebrewWitnesses : [];
...
const renderStats = (tokens) => {
  ...
  els.statMorph.textContent = String(withMorph);
  els.statExplanation.textContent = String(withExplanation);
};
