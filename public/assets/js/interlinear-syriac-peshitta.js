...
const getWitnessFromData = (data) => {
  const literalEntry = (data.literalTranslations || []).find((entry) => entry.lang === "syriac");
  const transliterationFromTokens = (data.tokens || [])
    .filter((token) => token.lang === "syriac" && token.transliteration && token.transliteration !== "-")
    .map((token) => token.transliteration.trim())
    .filter(Boolean)
    .join(" ");

  const witnessList = Array.isArray(data.syriacWitnesses) ? data.syriacWitnesses : [];
  let witness = witnessList.find((item) => item.id === WITNESS_ID || item.id === "base") || witnessList[0];

  if (witness) {
    return {
      label: witness.label && witness.label !== "Texto-base" ? witness.label : "Síriaco Peshitta",
      text: witness.text || data.sourceTexts?.syriac || "Sem texto disponível.",
      transliteration:
  ...
  return {
    label: "Síriaco Peshitta",
    text: data.sourceTexts?.syriac || "Sem texto disponível.",
...
const getSyriacTokens = (data) => {
  const tokens = Array.isArray(data.tokens) ? data.tokens : [];
  return tokens.filter((token) => token.lang === "syriac");
};
...
const renderTokenRows = (tokens) => {
  ...
  if (!tokens.length) {
    ...
    tr.innerHTML = '<td colspan="8">Nenhum token síriaco disponível para este verso.</td>';
...
const renderVerse = (data) => {
  ...
  const tokens = getSyriacTokens(data);
