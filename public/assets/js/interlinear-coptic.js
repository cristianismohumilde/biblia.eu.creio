...
const getWitnessFromData = (data) => {
  const literalEntry = (data.literalTranslations || []).find((entry) => entry.lang === "coptic");
  const transliterationFromTokens = (data.tokens || [])
    .filter((token) => token.lang === "coptic" && token.transliteration && token.transliteration !== "-")
    .map((token) => token.transliteration.trim())
    .filter(Boolean)
    .join(" ");

  const witnessList = Array.isArray(data.copticWitnesses) ? data.copticWitnesses : [];
  let witness = witnessList.find((item) => item.id === WITNESS_ID) || witnessList[0];

  if (witness) {
    return {
      label: witness.label && witness.label !== "Texto-base" ? witness.label : "Testemunhos Coptas",
      text: witness.text || data.sourceTexts?.coptic || "Sem texto disponível.",
      transliteration:
  ...
  return {
    label: "Testemunhos Coptas",
    text: data.sourceTexts?.coptic || "Sem texto disponível.",
...
const getCopticTokens = (data) => {
  const tokens = Array.isArray(data.tokens) ? data.tokens : [];
  return tokens.filter((token) => token.lang === "coptic");
};
...
const renderTokenRows = (tokens) => {
  ...
  if (!tokens.length) {
    ...
    tr.innerHTML = '<td colspan="8">Nenhum token copta disponível para este verso.</td>';
...
const renderVerse = (data) => {
  ...
  const tokens = getCopticTokens(data);
