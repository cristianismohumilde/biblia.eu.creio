...
const getWitnessFromData = (data) => {
  const literalEntry = (data.literalTranslations || []).find((entry) => entry.lang === "latin");
  const transliterationFromTokens = (data.tokens || [])
    .filter((token) => token.lang === "latin" && token.transliteration && token.transliteration !== "-")
    .map((token) => token.transliteration.trim())
    .filter(Boolean)
    .join(" ");

  const witnessList = Array.isArray(data.latinWitnesses) ? data.latinWitnesses : [];
  let witness = witnessList.find((item) => item.id === WITNESS_ID || item.id === "base") || witnessList[0];

  if (witness) {
    return {
      label: witness.label && witness.label !== "Texto-base" ? witness.label : "Vulgata Latina",
      text: witness.text || data.sourceTexts?.latin || "Sem texto disponível.",
      transliteration:
  ...
  return {
    label: "Vulgata Latina",
    text: data.sourceTexts?.latin || "Sem texto disponível.",
...
const getLatinTokens = (data) => {
  const tokens = Array.isArray(data.tokens) ? data.tokens : [];
  return tokens.filter((token) => token.lang === "latin");
};
...
const renderTokenRows = (tokens) => {
  ...
  if (!tokens.length) {
    ...
    tr.innerHTML = '<td colspan="8">Nenhum token latim disponível para este verso.</td>';
...
const renderVerse = (data) => {
  ...
  const tokens = getLatinTokens(data);
